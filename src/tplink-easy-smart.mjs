const LINK_NAMES = { 0:'Link Down', 1:'LS 1', 2:'10M Half', 3:'10M Full', 4:'LS 4', 5:'100M Full', 6:'1000M Full' };
...
export function parsePortStatistics(text) {
  ...
  const ports=[];
  for (let i=0;i<maxPorts;i++) ports.push({port:i+1,enabled:state[i]===1,stateCode:state[i],linkCode:link[i],link:LINK_NAMES[link[i]]??`Unknown ${link[i]}`,up:link[i]!==0,txGood:pkts[i*4],txBad:pkts[i*4+1],rxGood:pkts[i*4+2],rxBad:pkts[i*4+3]});
  return ports;
}

function cookieHeader(headers) {
  const raw=headers.get('set-cookie'); if(!raw) return '';
  return raw.split(/,(?=[^;,]+=)/).map(x=>x.split(';',1)[0]).join('; ');
}

export async function pollEasySmart({host,username,password,timeoutMs=6000}) {
  const base=`http://${host}`;
  const login=await fetch(`${base}/logon.cgi`,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',referer:`${base}/Logout.htm`},body:new URLSearchParams({logon:'Login',username,password}),redirect:'manual',signal:AbortSignal.timeout(timeoutMs)});
  if(![200,302,303].includes(login.status)) throw new Error(`login HTTP ${login.status}`);
  const cookie=cookieHeader(login.headers);
  const stats=await fetch(`${base}/PortStatisticsRpm.htm`,{headers:{referer:`${base}/`,...(cookie?{cookie}:{})},signal:AbortSignal.timeout(timeoutMs)});
  if(!stats.ok) throw new Error(`statistics HTTP ${stats.status}`);
  return parsePortStatistics(await stats.text());
}
