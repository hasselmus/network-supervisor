...
import { ping, dnsProbe, wifiLink, fetchWitness } from './probes.mjs';
...
async function pollInterface(name) {
  ...
  const router = await ping(config.observer.router, name);
  const internet = await Promise.all((config.observer.internetTargets || []).map(async target => ({ target, ...await ping(target, name) })));
  const dns = await dnsProbe(config.observer.dnsServer, address).catch(err => ({ ok: false, error: String(err) }));
  return [name, { configured: true, address, router, internet, dns }];
}
